...
var desiredMapCost = 0;
var desiredMapBought = true;
...
function canAffordMap() {
	return game.resources.fragments.owned >= updateMapCost(true);
}
...
function autoMap() {
	...
	for (var map in game.global.mapsOwnedArray) {
		if (!game.global.mapsOwnedArray[map].noRecycle) {
			var mapLevel = game.global.mapsOwnedArray[map].level;
			if (mapLevel < game.global.world - 10) {
				recycleMap(map);
				break;
			}
	...
	if (!game.global.preMapsActive && game.global.mapsActive) {
		...
		if (selectedMap == game.global.currentMapId && (!getCurrentMapObject().noRecycle && (doDefaultMapBonus || vanillaMapatZone || doMaxMapBonus || shouldFarm || needPrestige || shouldDoSpireMaps))) {
			...
			if (shouldFarm && desiredMapBought == false && (desiredMapCost <= game.global.fragments)) {
				repeatClicked();
				selectedMap = "create";
				debug("Abandoning current map, can craft better farming map", "maps");
			}
		} else {
			if (game.global.repeatMap) {
				repeatClicked();
		...
		if (selectedMap == "world") {
			...
			if (getPageSetting('AdvMapSpecialModifier')) {
				Object.keys(mapSpecialModifierConfig).forEach(function (o) {
					...
					if (game.global.highestLevelCleared + 1 >= p.unlocksAt)
						availableSpecialModifiers.push(p.abv.toLowerCase());
				});
				if (availableSpecialModifiers.length > 0 && game.global.highestLevelCleared >= 59) {
					var c = document.getElementById("advSpecialSelect");
					if (c) {
						if (needPrestige && availableSpecialModifiers.includes("p")) {
							c.value = "p";
							decrement.push('specToFA');
						} else if (shouldFarm || !enoughHealth || preSpireFarming) {
							c.value = availableSpecialModifiers.includes("lmc") ? "lmc" : availableSpecialModifiers.includes("hc") ? "hc" : availableSpecialModifiers.includes("smc") ? "smc" : "lc";
							decrement.push('specToFA');
						} else {
							c.value = "fa";
						}
					}
					//Higher level maps disabled for now.
					//if (209 <= game.global.highestLevelCleared) {
					//	var m = document.getElementById("advExtraLevelSelect");
					//	if (m) {
			...
			desiredMapCost = updateMapCost(true);

			var maplvlpicked = parseInt(mapLevelInput.value) + (getPageSetting('AdvMapSpecialModifier') ? getExtraMapLevels() : 0);
			if (!(canAffordMap())) {
				debug("Can't afford the map we designed, #" + maplvlpicked + ", cost is " + (100 * (desiredMapCost / fragments)).toFixed(2) + "% of fragments available (" + prettify(updateMapCost(true)) + ")", "maps", '*crying2');
				desiredMapBought = false;
				// Too expensive, start decreasing costs
				while (decrement.indexOf('diff') > -1 && difficultyAdvMapsRange.value > 0 && !(canAffordMap())) {
					difficultyAdvMapsRange.value -= 1;
					console.log("Decreasing difficulty slider (" + prettify(updateMapCost(true)) + ")");
				}
				while (decrement.indexOf('loot') > -1 && lootAdvMapsRange.value > 0 && !(canAffordMap())) {
					lootAdvMapsRange.value -= 1;
					console.log("Decreasing loot slider (" + prettify(updateMapCost(true)) + ")");
				}
				while (decrement.indexOf('size') > -1 && sizeAdvMapsRange.value > 0 && !(canAffordMap())) {
					sizeAdvMapsRange.value -= 1;
					console.log("Decreasing size slider (" + prettify(updateMapCost(true)) + ")");
				}
				if (decrement.indexOf('specToFA') > -1 && !(canAffordMap())) {
					document.getElementById("advSpecialSelect").value = "fa";
					console.log("Trying FA (" + prettify(updateMapCost(true)) + ")");
				}
				if (!(canAffordMap())) {
					document.getElementById("advSpecialSelect").value = "0";
					console.log("No special modifier (" + prettify(updateMapCost(true)) + ")");
				}
				while (lootAdvMapsRange.value > 0 && !(canAffordMap())) {
					lootAdvMapsRange.value -= 1;
					console.log("Decreasing loot slider (" + prettify(updateMapCost(true)) + ")");
				}
				while (difficultyAdvMapsRange.value > 0 && !(canAffordMap())) {
					difficultyAdvMapsRange.value -= 1;
					console.log("Decreasing difficulty slider (" + prettify(updateMapCost(true)) + ")");
				}
				while (sizeAdvMapsRange.value > 0 && !(canAffordMap())) {
					sizeAdvMapsRange.value -= 1;
					console.log("Decreasing size slider (" + prettify(updateMapCost(true)) + ")");
				}
			} else {
				desiredMapBought = true;
			}
			...
			if (!(canAffordMap())) {
				...
				debug("Buying a Map, level: #" + maplvlpicked + ", cost is " + (100 * (updateMapCost(true) / fragments)).toFixed(2) + "% of fragments available (" + prettify(updateMapCost(true)) + ")", "maps", 'th-large');
				var result = buyMap();
				if (result == -2) {
